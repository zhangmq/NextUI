...
// Per-output-frame statistics for the present paths: GFX_flip / GFX_GL_Swap
// sample exactly once per presented frame here. Besides the debug-HUD fields
// (perf.fps / avg_frame_ms / max_frame_ms / frame_drops / jitter) this also
// owns current_fps, which SND_batchSamples divides by when resampling core
...
// blocking audio write). Every core takes this path (ma_audio.c sets
// block_on_full unconditionally) -- only the fixed-rate variant drops
// instead of waiting, and that variant is explicitly not a pace.
...
size_t SND_batchSamples(const SND_Frame *frames, size_t frame_count)
{
	...
	// NOTE: with block_on_full (every core now) the audio device must keep
