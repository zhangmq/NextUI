...
void runShaderPass(ShaderPass * shader_pass, GLuint src_texture,
	...
	float uv_w = (shader_pass->texw > 0) ? (float)shader_pass->srcw / shader_pass->texw : 1.0f;
	float uv_h = (shader_pass->texh > 0) ? (float)shader_pass->srch / shader_pass->texh : 1.0f;
	...
	if (shader_program->u_MVP >= 0 && mvp) {
		glUniformMatrix4fv(shader_program->u_MVP, 1, GL_FALSE, mvp);
	}
	...
	if (target_texture) {
		...
		if (*target_texture==0 || shader_pass->target_updated || reloadShaderTextures
			...
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
			// RA pass FBOs are pow2-sized (fbo_rect width/height =
...
void PLAT_run_shader_pipeline(GLuint src_texture, GLuint orig_texture_src,
	...
	for (int i = 0; i < nrofshaders; i++) {
		...
		last_w = pass_dst_w;
		last_h = pass_dst_h;
		// Chain pass FBOs are pow2-sized (runShaderPass): the next pass's
		// source texture size is that pow2, its content the pass dims.
	...
	final_pass->texw = (int)last_tex_w;
	final_pass->texh = (int)last_tex_h;
