...
#define GFX_run_shader_pipeline PLAT_run_shader_pipeline // (src, orig, fw, fh, final_noflip, dx, dy, dw, dh, tex_w, tex_h, rotation, nextui_mode)
...
// RA-aligned shader-chain pipeline (gl2_renderchain_render). src_tex_w/h:
// the source TEXTURE dims (hw-render: the pow2 core FBO; software: the
// frame-sized upload); rotation: SET_ROTATION index (0-3) applied to the
// final pass MVP only (chain passes run in the core's orientation).
...
// RA gl2_set_projection: mvp = Rz(90*rotation) * ortho(0,1,0,1,-1,1),
// column-major (glUniformMatrix4fv GL_FALSE). Shared by the direct quad
// present and the chain final pass.
void PLAT_compute_present_mvp(int rotation, float mvp[16]);
// RA video_viewport_get_scaled_* model: Screen Scaling (Native / Aspect /
// Aspect Screen / Fullscreen / Cropped) -> the present rect in device pixels.
