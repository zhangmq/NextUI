...
// ---------------------------------------------------------------------------
// External pads (minarch-gl overlay; see workspace/tmp/pad-probe/).
//
// Upstream reads every /dev/input/eventN through the *built-in* pad's evdev
// table and every SDL joystick through one hardcoded JOY_* index table, with
// the analog axes gated on the SKU's own sticks.  A real external pad is
// therefore read twice under two wrong interpretations (buttons land on other
// BTNs, LT/RT arrive as axes 2/5 and are dropped, sticks are gated off).
//
// This copy adds a pad-aware path, default on and switchable off for A/B:
//   NEXTUI_EXTERNAL_PAD=legacy  -> byte-for-byte upstream behaviour
//   NEXTUI_EXTERNAL_PAD unset/auto -> pads SDL recognises as game controllers
//     go through SDL_GameController (normalised A/B/X/Y, shoulders, sticks,
//     triggers, dpad, guide) and their evdev node is skipped so the pad is
//     read exactly once; pads SDL does not recognise keep the legacy path.
//     A mapping file may describe further pads:
//       $USERDATA_PATH/gamecontrollerdb.txt, $SYSTEM_PATH/gamecontrollerdb.txt
//     (standard SDL gamecontrollerdb lines; no minarch cfg key involved).
//   NEXTUI_EXTERNAL_PAD_DEBUG=1 -> trace the pad's contribution to the state
//     cores read (pad.laxis/raxis and the digital triggers).
//
// The built-in pad is untouched: it stays on raw evdev and is still skipped by
// the SDL enumeration (is_builtin_pad), as upstream.
//
// Scope -- this is deliberately *BASIC* external pad support:
//   * GUID-based: a pad only gets the normalised path when gamecontrollerdb.txt
//     has a line for its GUID.  The SDL build shipped on this device has no
//     usable built-in pad DB (measured: an official Xbox 360 GUID reports
//     is_gc=0), so in practice this means "the pads with an entry", NOT "all
//     XInput pads".  Nothing is generated at runtime yet.
//   * the mapping line assumes the standard evdev gamepad layout
//     (BTN_SOUTH/EAST/NORTH/WEST/TL/TR/SELECT/START/MODE/THUMBL/THUMBR,
//     ABS_X/Y/Z/RX/RY/RZ, HAT0).  Pads whose triggers are buttons
//     (BTN_TL2/TR2), whose d-pad is buttons, or whose button order differs
//     need their own line.
//   * analog triggers are digitised (>16384 press, <8192 release): cores see
//     JOYPAD L2/R2, not trigger axes.
//   * one shared PAD state: everything is player 1.  No per-port assignment,
//     no per-pad remap UI or persistence beyond the mapping file.
//   * pads SDL does not recognise keep the legacy path (read twice, axes gated
//     on the SKU's own sticks) -- i.e. still broken until they get a line.
//   * minarch-gl only: the launcher (nextui), minput, settings and the stock
//     minarch target are untouched, so an external pad still does not drive
//     the launcher menu.
//   * NEXTUI_EXTERNAL_PAD / NEXTUI_EXTERNAL_PAD_DEBUG are test-only knobs with
//     no UI.
// ---------------------------------------------------------------------------
#define EXT_MAX_PADS 4
typedef struct {
...
static ExtPad ext_pads[EXT_MAX_PADS];
static int num_ext_pads = 0;
static int ext_mode = -1; // 0 = legacy (upstream), 1 = auto

static int ext_auto_mode(void) {
	if (ext_mode < 0) {
		const char *v = getenv("NEXTUI_EXTERNAL_PAD");
		ext_mode = (v && v[0] && !strcasecmp(v, "legacy")) ? 0 : 1;
	}
	return ext_mode;
}

static ExtPad *ext_pad_by_id(SDL_JoystickID id) {
	for (int i = 0; i < num_ext_pads; i++)
		if (ext_pads[i].id == id) return &ext_pads[i];
	return NULL;
}

static int ext_pad_owns_path(const char *path) {
	if (!ext_auto_mode() || !path || !path[0]) return 0;
	for (int i = 0; i < num_ext_pads; i++)
		if (ext_pads[i].path[0] && !strcmp(ext_pads[i].path, path)) return 1;
	return 0;
}

// SDL_GameControllerAddMappingsFromFile() silently ignores files it cannot
// parse as a DB (measured: a one-entry file returns 0 and adds nothing), so
// read the file here and hand SDL one line at a time -- then the file stays a
// plain text config (comments, blank lines and CRLF all tolerated).
static int ext_load_mapping_file(const char *path) {
	FILE *f = fopen(path, "r");
	if (!f) return 0;
	char line[512];
	int added = 0;
	while (fgets(line, sizeof(line), f)) {
		...
		while (n && (p[n - 1] == '\n' || p[n - 1] == '\r')) p[--n] = '\0';
		if (!n) continue;
		if (SDL_GameControllerAddMapping(p) > 0) added++;
		else LOG_error("Bad controller mapping line in %s: %s\n", path, p);
	}
	fclose(f);
	return added;
}

static void ext_load_mappings(void) {
	if (!ext_auto_mode()) return;
	int u = ext_load_mapping_file(USERDATA_PATH "/gamecontrollerdb.txt");
	int y = ext_load_mapping_file(SYSTEM_PATH "/gamecontrollerdb.txt");
	if (u || y) LOG_info("External pad mappings: %d (user) + %d (system)\n", u, y);
}

static void ext_pad_open(int device_index) {
	if (num_ext_pads >= EXT_MAX_PADS) return;
	SDL_GameController *gc = SDL_GameControllerOpen(device_index);
	if (!gc) {
		LOG_error("Failed to open controller %d: %s\n", device_index, SDL_GetError());
		return;
	}
	ExtPad *p = &ext_pads[num_ext_pads++];
	p->gc = gc;
	...
	LOG_info("Controller added: %s path=%s id=%i\n", SDL_GameControllerName(gc), p->path, p->id);
}

static void ext_pad_close(SDL_JoystickID id) {
	for (int i = 0; i < num_ext_pads; i++) {
		if (ext_pads[i].id != id) continue;
		LOG_info("Controller removed: %s\n", SDL_GameControllerName(ext_pads[i].gc));
		SDL_GameControllerClose(ext_pads[i].gc);
		for (int k = i; k < num_ext_pads - 1; k++) ext_pads[k] = ext_pads[k + 1];
		num_ext_pads--;
		memset(&ext_pads[num_ext_pads], 0, sizeof(ExtPad));
		return;
	}
}

static int button_from_controller(int b, int *id) {
	switch (b) {
	case SDL_CONTROLLER_BUTTON_A: *id = BTN_ID_A; return BTN_A;
	case SDL_CONTROLLER_BUTTON_B: *id = BTN_ID_B; return BTN_B;
	case SDL_CONTROLLER_BUTTON_X: *id = BTN_ID_X; return BTN_X;
	case SDL_CONTROLLER_BUTTON_Y: *id = BTN_ID_Y; return BTN_Y;
	case SDL_CONTROLLER_BUTTON_LEFTSHOULDER: *id = BTN_ID_L1; return BTN_L1;
	case SDL_CONTROLLER_BUTTON_RIGHTSHOULDER: *id = BTN_ID_R1; return BTN_R1;
	case SDL_CONTROLLER_BUTTON_BACK: *id = BTN_ID_SELECT; return BTN_SELECT;
	case SDL_CONTROLLER_BUTTON_START: *id = BTN_ID_START; return BTN_START;
	case SDL_CONTROLLER_BUTTON_GUIDE: *id = BTN_ID_MENU; return BTN_MENU;
	case SDL_CONTROLLER_BUTTON_LEFTSTICK: *id = BTN_ID_L3; return BTN_L3;
	case SDL_CONTROLLER_BUTTON_RIGHTSTICK: *id = BTN_ID_R3; return BTN_R3;
	case SDL_CONTROLLER_BUTTON_DPAD_UP: *id = BTN_ID_DPAD_UP; return BTN_DPAD_UP;
	case SDL_CONTROLLER_BUTTON_DPAD_DOWN: *id = BTN_ID_DPAD_DOWN; return BTN_DPAD_DOWN;
	case SDL_CONTROLLER_BUTTON_DPAD_LEFT: *id = BTN_ID_DPAD_LEFT; return BTN_DPAD_LEFT;
	case SDL_CONTROLLER_BUTTON_DPAD_RIGHT: *id = BTN_ID_DPAD_RIGHT; return BTN_DPAD_RIGHT;
	}
	*id = -1;
	return BTN_NONE;
}

// Trigger axes are analog; the rest of this frontend speaks digital L2/R2
// (every core falls back to the JOYPAD bit).  Hysteresis so a resting trigger
// does not chatter.
static int ext_trigger_pressed(int axis, int value) {
	int bit = (axis == SDL_CONTROLLER_AXIS_TRIGGERLEFT) ? BTN_L2 : BTN_R2;
	int threshold = (pad.is_pressed & bit) ? 8192 : 16384;
...
static void scan_evdev_inputs(void) {
	...
	for (int i = 0; i < H700_INPUT_COUNT; i++) {
		char path[64];
		snprintf(path, sizeof(path), "/dev/input/event%i", i);
		if (ext_pad_owns_path(path)) { // read once, through SDL
			close_evdev_input(i);
			continue;
		}
...
void PLAT_initInput(void) {
	...
	ext_load_mappings();
	if(SDL_InitSubSystem(SDL_INIT_JOYSTICK | (ext_auto_mode() ? SDL_INIT_GAMECONTROLLER : 0)) < 0)
    ...
    if (total > 0) {
        ...
        for (int i = 0; i < total; i++) {
			...
			if (ext_auto_mode() && SDL_IsGameController(i)) {
				ext_pad_open(i); // normalised mapping; evdev node skipped
				continue;
			}
...
void PLAT_quitInput(void) {
	...
	for (int i = 0; i < num_ext_pads; i++)
		if (ext_pads[i].gc) SDL_GameControllerClose(ext_pads[i].gc);
	num_ext_pads = 0;
	memset(ext_pads, 0, sizeof(ext_pads));
...
static void poll_sdl_input(uint32_t tick) {
	...
	while (SDL_PollEvent(&event)) {
		...
		if (event.type == SDL_JOYBUTTONDOWN || event.type == SDL_JOYBUTTONUP) {
			if (ext_pad_by_id(event.jbutton.which)) continue; // controller path owns it
			...
			btn = button_from_controller(event.cbutton.button, &id);
		}
		else if (event.type == SDL_CONTROLLERAXISMOTION) {
			int axis = event.caxis.axis;
			int val = event.caxis.value;
			// NEXTUI_EXTERNAL_PAD_DEBUG=1 traces what the pad contributes to
			// the state cores read (pad.laxis/raxis and the digital triggers).
			static int ext_debug = -1;
			if (ext_debug < 0) ext_debug = getenv("NEXTUI_EXTERNAL_PAD_DEBUG") != NULL;
			if (axis == SDL_CONTROLLER_AXIS_LEFTX) {
				pad.laxis.x = val;
				PAD_setAnalog(BTN_ID_ANALOG_LEFT, BTN_ID_ANALOG_RIGHT, val, tick + PAD_REPEAT_DELAY);
				if (ext_debug) LOG_info("EXTPAD axis %i=%i -> laxis=(%i,%i) raxis=(%i,%i)\n", axis, val, pad.laxis.x, pad.laxis.y, pad.raxis.x, pad.raxis.y);
				continue;
			}
				...
				btn = BTN_L2; id = BTN_ID_L2; pressed = ext_trigger_pressed(axis, val);
				if (ext_debug) LOG_info("EXTPAD trigger L axis %i=%i -> %s\n", axis, val, pressed ? "down" : "up");
			}
			else if (axis == SDL_CONTROLLER_AXIS_TRIGGERRIGHT) {
				btn = BTN_R2; id = BTN_ID_R2; pressed = ext_trigger_pressed(axis, val);
				if (ext_debug) LOG_info("EXTPAD trigger R axis %i=%i -> %s\n", axis, val, pressed ? "down" : "up");
			}
		}
		else if (event.type == SDL_JOYHATMOTION) {
			if (ext_pad_by_id(event.jhat.which)) continue;
			...
			if (ext_pad_by_id(event.jaxis.which)) continue;
...
void PLAT_updateInput(const SDL_Event *event) {
    ...
    case SDL_JOYDEVICEADDED: {
        ...
        if (ext_auto_mode() && SDL_IsGameController(device_index)) {
            ext_pad_open(device_index);
            break;
        }
    ...
    case SDL_JOYDEVICEREMOVED: {
        ...
        if (ext_pad_by_id(removed_id)) {
            ext_pad_close(removed_id);
            break;
        }
        for (int i = 0; i < num_joysticks; ++i) {
